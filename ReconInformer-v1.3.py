...
#==========================================================
#Feb 15, 2021
#v1.3 added: using -t flag we can process packets from a specific inbound IP address of interest.
#v1.3 added timestamp for the detection results in console output window.
#v1.3 fix: utf-8 directive, for encoding error encountered in some older versions of Python.
#v1.3 fix: check for where window size is not relevant for UDP packets to prevent errors.
#v1.3 removed: script name in console window title to view CL arguments.
#v1.3 suppressed: output for fragmented packets that don't show any useful info.
#For best realtime console output call ReconInformer using python -u ReconInformer.py <args>...
#
...
def doit(pkt):
    ...
    global gw, no_report_scan_list, dst_port_whitelist, scan_detect_lst, only_this_ip
    ...
    #Deal with ping sweep -sn -sP (if attacker uses -Pn it may fail)
    try:
        if pkt.haslayer(ARP):
            addr = str(pkt[ARP].psrc)
            mac = str(pkt[Ether].src)
            print(colored.red("[+] Recon:"+" "*(len("ARP Ping sweep")-1)+"IP:"+" "*(len(addr)+2)+"MAC:"+" "*(len(mac)+1)+"Time:"))
            print(colored.cyan("[*] ARP Ping sweep" +"  |  " + addr + "  |  " + str(mac) + "  "*1 + "|  " + detection_time()))
        ...
        if str(pkt.haslayer(ICMP)):
            if str(pkt.getlayer(ICMP).type) == "8":
                print(colored.cyan("[*] Ping from: "+pkt[0][IP].src + " | "+"Time: " + detection_time()))
                print(colored.red("-"*lines))
                sys.stdout.flush()
    except Exception as e:
        sys.stdout.flush()
    ...
    try:
        ...
        #Check if use -t flag.
        if only_this_ip != "" and addr != only_this_ip:
            return
    ...
    if fragmented==True:
        ...
        try:
            if pnum != "":
                print(colored.red("[+] Recon:"+" "*(len("Frag")-1)+"IP:"+" "*(len(addr)+2)+"MAC:"+" "*(len(mac)+1)+"Port:"+" "*(len(pnum)+2)+"Time:"))
                print(colored.cyan("[+] Frag" +  "  |  " + addr + "  |  " + str(mac) + "  |  " + pnum + "  "*2 + "|  " + detection_time()))
                      
                sys.stdout.flush()
            #Suppress output fragmented packets without meaningful info, as it just clutters the console anyways.
            else:
                print(colored.red("*"))
                ##print(colored.red("[+] Recon:"+" "*(len("Frag")-1)+"IP:"+" "*(len(addr)+2)+"MAC:"+" "*(len(mac)+1)+"Time:"))
                ##print(colored.cyan("[*] Frag" +"  |  " + addr + "  |  " + str(mac)+ "  "*2 + "|  " + detection_time()))

            lines = lines + len(pnum) + 2
            print(colored.red("-"*lines))
            sys.stdout.flush()
    ...
    print(colored.red("[+] Recon:"+" "*(len(SCAN_TYPE)-1)+"IP:"+" "*(len(addr)+2)+"MAC:"+" "*(len(mac)+1)+"Port:"+" "*(len(pnum)+2)+"Time:"))
    print(colored.green("[+] "+SCAN_TYPE +  "  |  " + addr + "  |  " + str(mac) + "  |  " + pnum + "  "*2 + "|  " + detection_time()))
    sys.stdout.flush()
...
def parse_args():
    ...
    parser.add_argument("-t", "--process_specific_ip", help="Process packets only from a IP of interest and nothing else. <-t x.x.x.x>")
...
def main(args):
    ...
    global pcap_file, recon_win, gw, no_report_scan_list, scan_detect_lst, only_this_ip
    ...
    print(colored.red("[*] Best realtime output use: python -u"))
    ...
    if args.ping_sweep:
        print(colored.cyan("[!] See your using -p, most Nmap scans start with ARP, if the attacker uses -Pn detection may fail."))
    ...
    if OS=="win32" and len(scan_detect_lst)==0:
        print(colored.cyan("[!] FIN,NULL,XMAS,MAIMON scans are ignored on Windows, still wish to detect them? use -s flag, see -h."))

    if args.whitelist and not args.block_mode:
        (colored.cyan("[!] -w has no block mode (-b)."))
        exit()
    ...
    #Process only packets from a certain host.
    if args.process_specific_ip:
        only_this_ip = args.process_specific_ip
